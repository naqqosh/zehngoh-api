...
export class ProductsService {
  ...
  async getById(id: number) {
    ...
    for (const variant of product.variants) {
      for (const attr of variant.attributes ?? []) {
        ...
        if (!attrEntry.values.has(attr.valueId)) {
          ...
          !attrEntry.values.get(attr.valueId)!.images.length &&
          attributeImagesMap.has(attr.valueId)
        ) {
          attrEntry.values.get(attr.valueId)!.images =
            attributeImagesMap.get(attr.valueId) ?? [];
        }
      }
    }
    ...
    const variants = product.variants.map((variant: any) => ({
      ...
      discountFixedUzs:
        variant.discountFixedUzs != null
          ? (variant.discountFixedUzs as unknown as bigint).toString()
          : "0",
    ...
    return {
      ...
      category: {
        ...
        nameUz: product.category.nameUz,
